...
%   of continuous data should be input as a separate cell and
%   MTRFATTNCROSSVAL zero-pads or truncates the trial boundaries
%   appropriately.
%
...
for i = 1:nfold
    ...
    if arg.verbose
        ...
        if i == nfold
            fprintf('] - %.2fs/fold\n',tocs/nfold)
            ravg = max(max(max(mean(r1,1))));
            erravg = min(min(min(mean(err1,1))));
            fprintf('STIM1: correlation: %.4f - error: %.4f\n',ravg,erravg)
            ravg = max(max(max(mean(r2,1))));
            erravg = min(min(min(mean(err2,1))));
            fprintf('STIM2: correlation: %.4f - error: %.4f\n',ravg,erravg)
        end
    end
...
% Print progress
if arg.verbose
    accavg = max(max(max(acc)));
    davg = max(max(max(d)));
    fprintf('accuracy: %.4f - d'': %.4f\n',accavg,davg)
end
